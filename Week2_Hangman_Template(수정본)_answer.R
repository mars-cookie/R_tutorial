...
play.hangman <- function() {
  # 변수 사전을 정의해 보세요.
  number.mistakes <-  0 # 문제 2. 틀린 횟수는 초기에 몇 번이 되어야 할까요?
  hangman.word <- "korea"
  ...
  while (number.mistakes<6)  { # 문제3. 실수가 6번까지 허용됩니다. 어떤 조건을 사용해야 할까요?
    cat(player, sep = '')  # 먼저 답안지를 출력.
    letter <- readline("정답을 입력해보세요 : ") # 문제 4. 사용자가 추측하도록 유도하는 메시지를 작성해 보세요. 
    ...
    while(toupper(letter) %in% used.letters) {  # 이미 사용했던 알파벳이라면? 어떤 제어문을 사용할까요?
      guess.messages <- cat(paste("이미 사용된 알파벳입니다.", toupper(letter)))  # 5. 사용자에게 알려주세요!
  ...
  if(!'_' %in% player) {
    cat('승리!')
    cat('정답은',hangman.word,"입니다.")
    break
   }
  }
  if(number.mistakes>=6)  { # 문제7. 만약 실수가 7번 이상이라면?
    # 문제8. guess.messages에 game over message를 작성해봅시다.
    # "Game Over ! 정답은 다음과 같습니다: korea"   (korea 그 자체로 쓰지 말고, 정답 단어 활용)
